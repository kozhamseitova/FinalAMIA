...
<div class="lg:w-2/3 w-full mx-auto overflow-auto">
    <table class="table-auto w-full text-left whitespace-no-wrap">
        ...
        <c:forEach items="${news1}" var="new1">
            <tr>
                <td class="px-4 py-3">${new1.id}</td>
                <td class="px-4 py-3">${new1.title}</td>
                <td class="px-4 py-3">${new1.description}</td>
                <td class="px-4 py-3">${new1.time}</td>
                <%
                    if(session.getAttribute("status")!=null){
                %>
                <td class="px-4 py-3"><a id="r" href="<%=request.getContextPath()%>/new?act=remove&id=${new1.id}">remove</a></td>
            <%
                }
            %>
